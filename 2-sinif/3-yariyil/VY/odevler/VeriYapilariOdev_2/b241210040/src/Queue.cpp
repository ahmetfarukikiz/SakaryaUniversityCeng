...
{
    if (isEmpty() || silinecekAgac == nullptr) throw "Hata";
...
//her zaman 0 a göre dağıttığım bu versiyondan vazgeçtim
...
//         items[suankiIndex]->add(alinanAgac[i]); 

//         suankiIndex = (suankiIndex + 1) % length; 
//     }
// }
...
Bst *Queue::peekMax()
{
    ...
    for (int i = 1; i < length; i++) {
        ...
        int currenth = currentBst->height();
        
        if (currenth > heightMax) {
            heightMax = currenth;
            bstMax = currentBst;
        }
    }
